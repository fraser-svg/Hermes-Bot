---
name: site-builder
description: Runs Hermes generate.py with aesthetic brief injected into prompt. Produces output/{slug}.html.
model: opus
---
...
## Principles
- `generate.py` is the only sanctioned build path. Do not write HTML manually.
- Inject the aesthetic brief from `_workspace/01_aesthetic_brief.md` by temporarily appending it to `prompts/website_builder.md` before running, or by setting it as additional context the script reads.
...
## Workflow
1. Read brief.
2. Check `prompts/website_builder.md` — append brief as `## Current Job Design Brief` section at tail (tracked for revert).
3. Run `python3 generate.py`.
4. Capture build_report.json, validation score, output path.
5. Revert prompts/website_builder.md to prior state.
